...
## Current status (latest release: v0.17.3-fork)

Everything works end-to-end and is shipping. The app is fully **rebranded to ByteLauncher** (Modrinth branding removed per `COPYING.md` — see the Branding section) and defaults to a purple theme. Releases ship a standalone `ByteLauncher.exe` **plus** two NSIS installers (`ByteLauncher-Setup.exe` online / `ByteLauncher-Setup-Offline.exe`); the app updates itself in-app — one-click from the Settings page **and** the startup banner. Recent fork.N history: fork.7 removed the core "already running" guard so relaunch works; **fork.8 = the ByteLauncher de-brand** (name/exe/icons/theme; bundle `identifier` kept `ModrinthApp` for zero data loss); fork.9 added the Purple theme (default) + fixed green accents; fork.10 = ByteBuilders Hosting "coming soon" page + more green fixes; **fork.11 = one-click update banner + the online/offline installers**; **fork.19 = two new opt-in built-in plugins: Hide News + Home Dashboard (Experimental)**; **fork.20 = the ByteBuilders Hosting (Experimental) plugin — the panel is back, now as an iframe through a header-stripping proxy (no native webview → no overlay bug) with a Main/SuperS/Kia panel picker** (see the Built-in plugins list); **fork.21 = fix panel login ("CSRF token mismatch") by disabling WebView2 tracking prevention on the main webview so third-party iframe cookies work**; **fork.22 = move the hosting proxy to subdomain-per-panel so the panel SPA routes at root and no longer 404s on tab-reopen**; **fork.23–24 = single-label proxy subdomains for free SSL; landed on a dedicated `<id>.bytebuilder.co.za` domain (fork.24)**; **fork.25 = Hide-News "hide entire section" option; fork.26 = plugin themes (plugins can add a selectable color theme) + the Rebrand Template builtin**. (fork.12–18 were DB self-heal, purple-default fix, installer download progress/rate-limit fixes, the Uninstall danger-zone, and reverting Hosting to "Coming soon".) After fork.26 the daily sync merged upstream **v0.15.11** (so the fork.N counter restarted on that base): **v0.15.11-fork.1 = route the in-app update *check* through a Cloudflare Worker** (`https://cloudflare-api.bytebuilders.co.za/bytelauncher/`) instead of `api.github.com`, so users can never hit GitHub's 60/hr API rate limit — see the In-app self-updater section. **v0.15.11-fork.2 = plugin list search/filter (All/Enabled/Disabled/New)/sort (Name/Recently updated) + a "New" badge (backed by a new `updated_at` mtime field on `read_plugins`), plus a fix to the theme selector wrongly highlighting Purple when a plugin theme is active** — the highlight now reads `themeStore.selectedTheme` (the source of truth) and plugin-theme picks persist in `localStorage` (`bytelauncher-plugin-theme`, never `app.db`) so they survive a restart (the backend `theme` enum can't store plugin theme names). Then the daily sync jumped upstream **v0.15.11 → v0.15.17** (6 releases at once); the only real conflict was `instance/Index.vue`, where upstream extracted the instance header into a new `InstancePageHeader` component — the Multi-Launch patch was re-applied via an `allowMultiLaunch` prop on that component (the extra Play button now lives in `apps/app-frontend/src/components/ui/instance-page-header/index.vue`). A 13-point verification workflow confirmed every fork patch survived the merge. New base tag: **v0.15.17-fork** (future feature releases are `v0.15.17-fork.N`). The following syncs (v0.15.18, **v0.15.19**) were the recurring *deleted-inherited-workflow* conflicts — resolved and shipped as **v0.15.19-fork**, and `fork-sync.yml` was hardened to **auto-resolve** that conflict class going forward (see CI, below), so it no longer needs manual intervention. That hardening then **proved itself: v0.15.20 auto-synced with zero manual work** (the auto-resolver `git rm`-ed its deleted-workflow conflicts, completed the merge, and pushed `v0.15.20-fork`). The next release, **v0.16.1** (a minor bump adding upstream's *shared instances* feature), produced a genuine **content** conflict — the auto-resolver correctly **escalated** it (opened an issue) instead of pushing a broken merge. Resolved manually by keeping **both** the fork's Multi-Launch (`allowMultiLaunch`) and upstream's new `sharedInstanceManager` prop/UI on the `InstancePageHeader` component (plus the usual keep-ByteLauncher-wording on 3 locale strings); shipped as **v0.16.1-fork** (new base tag; future feature releases are `v0.16.1-fork.N`). The next sync was the biggest yet: upstream jumped **two minor versions at once (v0.16.1 → v0.17.3)** including a **settings-layout refactor (#6891)** that moved the settings components into `settings/{account,display,instances}/` subdirs. The auto-resolver cleared the 15 deleted-workflow conflicts, then escalated **~11 genuine content conflicts** across core fork-patched files. Resolved by keeping every fork feature over upstream's refactor: **AppearanceSettings** moved to `settings/display/` (theme-selector fix re-applied there); **AppSettingsModal** adopted upstream's categorized-tab structure + re-added the fork **Plugins** tab (`category: display`) and kept the "Built on Modrinth" footer; **App.vue** kept the `app-brand-logo` statusbar + `NewsPanel` + hosting tooltip but took upstream's `<SurveyPopup />` (its script vars were removed) — and needed a re-added `LeftArrowIcon` import; **Servers.vue** kept the hosting iframe + added the new provider breadcrumb; **theme.ts/settings.rs** kept purple + added upstream's `retro`; **pages** kept plugin hooks + adopted `useRootBreadcrumb` (dropping the removed `@/store/breadcrumbs`). Two verification passes (fork-hooks + vue-tsc-level undefined-ref hunt) confirmed it; shipped as **v0.17.3-fork** (new base tag; future feature releases are `v0.17.3-fork.N`).
...
- **Multi-Launch** (opt-in, native — manifest-only, no JS) — when `playing && enabledPluginIds.has('multi-launch')`, a native **Play** button appears next to **Stop** to launch another copy. `apps/app-frontend/src/pages/instance/Index.vue` passes `:allow-multi-launch="enabledPluginIds.has('multi-launch')"` to the `InstancePageHeader` component (`apps/app-frontend/src/components/ui/instance-page-header/index.vue`, extracted by upstream in v0.15.12–17), which renders the extra Play button (emits `play` → `startInstance`). The core dedupe guard in `packages/app-lib/src/launcher/mod.rs` (which rejected launching an already-running instance) was **removed**; accidental double-fire is prevented by disabling Play while a launch is in flight.
...
**CI:** `.github/workflows/fork-build.yml` (validate — builds the standalone exe on push), `fork-release.yml` (build + publish the exe on a `v*` tag / dispatch), `fork-sync.yml` (daily; merges new upstream **releases** into `main`, tags `v<upstream>-fork`). All inherited Modrinth workflows were **deleted** (they need private Blacksmith runners / Modrinth secrets and just hang or fail on a fork). Because upstream edits those 16 deleted workflow files constantly, a "deleted by us" merge conflict on them was the recurring cause of sync failures — `fork-sync.yml` now **auto-resolves** deleted-by-us (`DU`) conflicts by `git rm`-ing them and completing the merge, and only aborts + opens an issue on a genuine **content** conflict (`UU`) that needs a human (e.g. a fork-patched file upstream refactored, like the v0.15.17 `Index.vue`→`InstancePageHeader` case).
